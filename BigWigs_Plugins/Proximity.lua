...
do
	...
	SlashCmdList.BigWigs_Proximity = function(input)
		...
		local inputTable = input and Split(input," ") or {}
		local range = tonumber(inputTable[1])
		if not range then
			plugin:Close()
			print("Usage: /proximity 1-100 <Player names (comma seperated) / all> <Is reverse (true)>")
		else
			local players = inputTable[2] and Split(inputTable[2],",") or nil
			if (players and #players == 1) then players = players[1]:lower() ~= "all" and players[1] end
			local isReverse = inputTable[3] == "true"
			if range > 0 then
				plugin:Open(range, nil, nil, players, isReverse)
			else
				plugin:Close()
			end
		end
